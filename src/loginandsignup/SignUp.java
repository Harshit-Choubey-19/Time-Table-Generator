...
public class SignUp extends javax.swing.JFrame {
    ...
    private void initComponents() {
        ...
        jLabel1.setIcon(new javax.swing.ImageIcon(getClass().getResource("/Icon/timetaabl1 (2).png"))); // NOI18N

        jLabel2.setFont(new java.awt.Font("Showcard Gothic", 0, 24)); // NOI18N
        jLabel2.setForeground(new java.awt.Color(255, 255, 255));
        jLabel2.setText("TIme table builder");
...
private boolean isValidEmail(String email) {
    ...
    if (fullName.isEmpty() || email.isEmpty() || password.isEmpty()) {
        JOptionPane.showMessageDialog(this, "All fields are required. Please fill in full name, email, and password.", "Input Error", JOptionPane.WARNING_MESSAGE);
        return;
    }
    
    // Check if email is valid
    if (!isValidEmail(email)) {
        JOptionPane.showMessageDialog(this, "Invalid email format. Please enter a valid email.", "Email Error", JOptionPane.ERROR_MESSAGE);
        return;
    }
    ...
    try {
        ...
        if (rs.next()) {
            JOptionPane.showMessageDialog(this, "Email already exists. Please use a different email.", "Sign Up Error", JOptionPane.ERROR_MESSAGE);
            return;
        }

        // Generate unique user ID (2022XXXXYYYY)
        String namePrefix = fullName.length() >= 4 ? fullName.substring(0, 4).toUpperCase() : fullName.toUpperCase();
        int randomNum = (int) (Math.random() * 9000) + 1000; // Generates a 4-digit random number
        String userId = "2022" + namePrefix + randomNum;

        // Insert new user into database
        String insertQuery = "INSERT INTO User (id, fullName, email, password) VALUES ('" + userId + "', '" + fullName + "', '" + email + "', '" + password + "')";
        int rowsInserted = stmt.executeUpdate(insertQuery);

        if (rowsInserted > 0) {
            JOptionPane.showMessageDialog(this, "Sign up successful! Your user ID: " + userId, "Success", JOptionPane.INFORMATION_MESSAGE);
            
            String findDept = "SELECT dept FROM User WHERE email = '" + email + "'";
            Statement stmt2 = conn.createStatement();
            ResultSet rs2 = stmt2.executeQuery(findDept);
            String dept = rs2.getString("dept");
            String role = rs2.getString("role");
            // Set user session
            UserSession.createSession(userId, fullName, email, dept, role);
